...
fn get_tabs(current_depth: u32) -> String {
    let tab_count = if current_depth > 10 {
        10
    } else {
        current_depth - 1
    };
...
pub fn decode_varint<W: Write>(
    ...
    let e_m = format!(
        "{}Parsing of a var_int started at offset {}\n",
        tabs, offset
    );
    ...
    loop {
        ...
        let &byte = bts.get(offset).ok_or_else(|| {
            PyValueError::new_err(format!(
                "While parsing var_int - no data to read at offset {}\n",
                offset
            ))
        })?;
        offset += 1;
...
fn decode_float<W: Write>(
    ...
    let tabs = get_tabs(current_depth);
    let e_m = format!(
        "{}Parsing of an 8-byte float started at offset {}\n",
        tabs, offset
    );
    result.write_all(e_m.as_bytes())?;
...
fn decode_optimized_float<W: Write>(
    buffer: &[u8],
    offset: usize,
    tag: Variant,
    result: &mut W,
    ...
    let tabs = get_tabs(current_depth);
    let e_m = format!(
        "{}Parsing of an optimized float started; expecting an int at offset {}\n",
        tabs, offset
    );
    result.write_all(e_m.as_bytes())?;
    ...
    if tag == Variant::FloatNoDecimals {
        let e_m = format!(
            "{}Positive float without decimals parsed: {}\n",
            tabs, value
        );
        result.write_all(e_m.as_bytes())?;
        ...
        let e_m = format!(
            "{}Negative float without decimals parsed: {}\n",
            tabs,
            -(value as f64)
        );
        result.write_all(e_m.as_bytes())?;
...
fn decode_string<W: Write>(
    ...
    if tag == Variant::String || tag == Variant::StringCompressed {
        let e_m = format!(
            "{}Expected integer (string size) at offset {}\n",
            tabs, offset
        );
        result.write_all(e_m.as_bytes())?;
        let (value, read) = decode_varint(buffer, offset, result, current_depth)?;
        ...
        let e_m = format!(
            "{}It is an optimized string, no need to parse its size; expected string size is {}\n",
            tabs,
            (tag as u8 - 40)
        );
        result.write_all(e_m.as_bytes())?;
...
fn decode_tuple<W: Write>(
    ...
    let e_m = format!(
        "{}Tuple with {} elements fully parsed\n",
        tabs, elements_count
    );
    res.write_all(e_m.as_bytes())?;
    Ok((result, new_offset))
...
fn decode_dict<W: Write>(
    ...
    let e_m = format!(
        "{}Dict with {} elements (pairs) fully parsed\n",
        tabs, elements_count
    );
    res.write_all(e_m.as_bytes())?;
    Ok((result, new_offset))
...
fn decode_cached_int<W: Write>(
    ...
    let tabs = get_tabs(current_depth);
    let e_m = format!(
        "{}Integer cache tag found; expecting a 1-byte cache index at offset {}\n",
        tabs, offset
    );
    res.write_all(e_m.as_bytes())?;
    if let Some(&index) = buffer.get(offset) {
        ...
        match opts.get_int(index) {
            Some(value) => {
                let e_m = format!(
                    "{}Retrieved from integer cache: index={}, value={}\n",
                    tabs, index, value
                );
                res.write_all(e_m.as_bytes())?;
        ...
        let e_m = format!(
            "Expect integer cache index, but nothing to read at offset {}\n",
            offset
        );
        Err(PyValueError::new_err(e_m))
    }
...
fn decode_cached_float<W: Write>(
    ...
    let tabs = get_tabs(current_depth);
    let e_m = format!(
        "{}Float cache tag found; expecting a 1-byte cache index at offset {}\n",
        tabs, offset
    );
    res.write_all(e_m.as_bytes())?;
    if let Some(&index) = buffer.get(offset) {
        ...
        match opts.get_float(index) {
            Some(value) => {
                let e_m = format!(
                    "{}Retrieved from float cache: index={}, value={}\n",
                    tabs, index, value
                );
                res.write_all(e_m.as_bytes())?;
        ...
        let e_m = format!(
            "Expect float cache index, but nothing to read at offset {}\n",
            offset
        );
        Err(PyValueError::new_err(e_m))
    }
...
fn decode_cached_string<W: Write>(
    ...
    let tabs = get_tabs(current_depth);
    let e_m = format!(
        "{}String cache tag found; expecting a 1-byte cache index at offset {}\n",
        tabs, offset
    );
    res.write_all(e_m.as_bytes())?;
    if let Some(&index) = buffer.get(offset) {
        ...
        match opts.get_string(index) {
            Some(value) => {
                let e_m = format!(
                    "{}Retrieved from string cache: index={}, value='{}'\n",
                    tabs, index, value
                );
                res.write_all(e_m.as_bytes())?;
...
fn decode<W: Write>(
    ...
    if let Some(&tag) = buffer.get(offset) {
        ...
        match Variant::try_from(tag) {
            ...
            Ok(Variant::CacheInt) => {
                decode_cached_int(buffer, new_offset, opts, result, current_depth)
            }
            Ok(Variant::CacheFloat) => {
                decode_cached_float(buffer, new_offset, opts, result, current_depth)
            }
            Ok(Variant::CacheString) => {
                decode_cached_string(buffer, new_offset, opts, result, current_depth)
            }
            Ok(Variant::DateTimeNoTz) => {
                ...
                let e_m = format!(
                    "{}Datetime without timezone (timestamp={}) parsed\n",
                    tabs, f
                );
                result.write_all(e_m.as_bytes())?;
            ...
            Ok(Variant::DateTimeOffset) => {
                ...
                let (f, new_offset) = parse_float(buffer, new_offset, opts, current_depth, result)?;
                let e_m = format!(
                    "{}Timestamp={} parsed. An integer (offset) is now expected.\n",
                    tabs, f
                );
                result.write_all(e_m.as_bytes())?;
                ...
                match pd {
                    ParsedData::Int(v) => {
                        let e_m = format!(
                            "{}Datetime with offset (timestamp={}, offset={}) parsed\n",
                            tabs, f, v
                        );
                        result.write_all(e_m.as_bytes())?;
            ...
            Ok(Variant::DateTimeIana) => {
                ...
                let (f, new_offset) = parse_float(buffer, new_offset, opts, current_depth, result)?;
                let e_m = format!(
                    "{}Timestamp={} parsed. A string (IANA) is now expected.\n",
                    tabs, f
                );
                result.write_all(e_m.as_bytes())?;
                let (pd, new_offset) = decode(buffer, new_offset, opts, current_depth, result)?;
                match pd {
                    ParsedData::String(v) => {
                        let e_m = format!(
                            "{}Datetime with IANA (timestamp={}, IANA='{}') parsed\n",
                            tabs, f, v
                        );
                        result.write_all(e_m.as_bytes())?;
                    ...
                    || (t >= Variant::FloatNoDecimalsNeg && t <= Variant::Float6Neg) =>
            {
                ...
                result.write_all(e_m.as_bytes())?;
                let (value, off) =
                    decode_optimized_float(buffer, new_offset, t, result, current_depth)?;
                let e_m = format!("{}Float={} parsed, pushing to cache\n", tabs, value);
                result.write_all(e_m.as_bytes())?;
                opts.add_float(value);
                Ok((ParsedData::Float(value), new_offset + off))
            ...
            Ok(t) if (t >= Variant::StringCompressed && t <= Variant::String15) => {
                ...
                let e_m = format!(
                    "{}String with len={} parsed, pushing to cache\n",
                    tabs,
                    value.len()
                );
                result.write_all(e_m.as_bytes())?;
            ...
            Ok(Variant::String) => {
                ...
                result.write_all(e_m.as_bytes())?;
                let (value, offset) =
                    decode_string(buffer, new_offset, Variant::String, result, current_depth)?;
                let e_m = format!(
                    "{}String with len={} parsed, pushing to cache\n",
                    tabs,
                    value.len()
            ...
            Ok(t) if t == Variant::List || (t >= Variant::List1 && t <= Variant::List10) => {
                ...
                result.write_all(e_m.as_bytes())?;
                let (value, offset) =
                    decode_list(buffer, new_offset, t, opts, current_depth + 1, result, true)?;
            ...
            Ok(t) if t == Variant::Tuple || (t >= Variant::Tuple2 && t <= Variant::Tuple5) => {
                ...
                result.write_all(e_m.as_bytes())?;
                let (value, offset) =
                    decode_tuple(buffer, new_offset, t, opts, current_depth + 1, result)?;
            ...
            Ok(s) if s == Variant::Set => {
                ...
                result.write_all(e_m.as_bytes())?;
                let (value, offset) =
                    decode_set(buffer, new_offset, s, opts, current_depth + 1, result)?;
            ...
            Ok(Variant::Dict) => {
                ...
                result.write_all(e_m.as_bytes())?;
                let (value, offset) =
                    decode_dict(buffer, new_offset, opts, current_depth + 1, result)?;
...
fn _explain<W: Write>(
    ...
    let e_m = format!(
        "Parsing started at offset {}, total length {}\n",
        offset, length
    );
    result.write_all(e_m.as_bytes())?;
    match decode(buffer, offset, opts, 1, result) {
        Ok((_, s)) => {
            ...
            if s < length {
                let e_m = format!(
                    "[ERROR] Corrupt data, finished at offset {}, but still have {} bytes unparsed, total data length {}\n",
                    s,
                    length - s,
                    length
                );
...
pub fn explains(buffer: Vec<u8>, offset: usize, max_depth: i32) -> PyResult<String> {
...
mod tests {
    ...
    fn test_dt_iana() {
        let b: Vec<u8> = vec![
            1, 29, 12, 65, 218, 168, 19, 252, 0, 0, 0, 53, 69, 117, 114, 111, 112, 101, 47, 77,
            111, 115, 99, 111, 119,
        ];
    ...
    fn test_long_string() {
        let b: Vec<u8> = vec![
            1, 13, 16, 116, 101, 120, 116, 116, 101, 120, 116, 116, 101, 120, 116, 116, 101, 120,
            116,
        ];
    ...
    fn test_compressed_string() {
        let b: Vec<u8> = vec![
            1, 40, 14, 120, 156, 43, 73, 173, 40, 41, 193, 131, 1, 233, 104, 14, 41,
        ];
    ...
    fn test_list_and_caches() {
        let b: Vec<u8> = vec![
            1, 86, 22, 186, 2, 44, 116, 101, 115, 116, 10, 232, 132, 1, 37, 0, 38, 0, 39, 0,
        ];
